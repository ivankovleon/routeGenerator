...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        if (!Yii::$app->user->isGuest) {
            ...
            $asd = 0;
            return $this->render('userIndex',['listDataProvider' => $dataProvider]);
        }
        return $this->render('index');
    }

    public function actionCustomizeMap($mapId, $pointStartx, $pointStarty)
    {
...
//        }
        ...
        $routeLength = $map->root_route_length;
        $controlPointsNumber = $map->control_points_number;
...
//                    $quarter = $quarterRandom;
    ...
    public function actionCreateRoute($mapId){
        $map = Map::findIdentity($mapId);
        if (Yii::$app->request->isAjax) {
            Yii::$app->response->format = Response::FORMAT_JSON;

            $map->control_points_number = (int)$_POST['pointsNumber'];
            $map->root_route_length = (int)$_POST['routeLength'];
            $map->map_scale = (int)$_POST['mapScale'];
            if($map->save()){
                $pointx = (int)$_POST['startPoint']['x'];
                $pointy = (int)$_POST['startPoint']['y'];
                $url = Url::to(['site/customize-map', 'mapId' => $map->id, 'pointx' => $pointx  ,'pointy' => $pointy]);
                return array(
                    'status' => 'success',
                    'href' => Url::toRoute(['site/customize-map', 'mapId' => $map->id, 'pointStartx' => $pointx  ,'pointStarty' => $pointy], true)
                );
            }

        }
        return $this->render('createRoute',['map' => $map]);
    }
//    public function actionC
    public function actionAddMap(){
        ...
        if($map->load(Yii::$app->request->post()) &&  $map->validate()) {
            ...
            if (!is_null($image)) {
                ...
                Image::thumbnail($path, 120, 120)
                    ->save(Yii::$app->params['uploadPath'].'sm_'.$map->file_name, ['quality' => 100]);
                if($map->save()) {
                    Yii::$app->getSession()->setFlash('success', 'Крата успешно добавлена');
                    return $this->actionCreateRoute($map->id);
                } else {
        ...
        return $this->render('addMap',['map'=>$map]);
    }
